...
function enhanceStep(step) {
  if (step.type === TYPE_SELECT) {
    step.hint = ' ';
  } else if (step.type === TYPE_MULTI_SELECT) {
    step.instructions = false;
    step.optionsPerPage = Infinity;
  }
  if (Array.isArray(step.choices)) {
    step.choices = step.choices.map((item) => (
      typeof item === 'string' ? {
        title: item,
        value: item,
      } : item
